...
class Handler:
    ...
    def route(self, req):
        ...
        if text.startswith('/start'):
            self.__handle_start(chat_id)
    
        if text.startswith('/play_bot'):
            self.__handle_play_bot(chat_id)

        if text.startswith('/play_online'):
            self.__handle_play_online(chat_id)
        
        if text.startswith('Close'):
            self.__handle_close(chat_id)

        if text in ['🪨', '✂️', '📃']:
            self.__handle_move(chat_id, text)

        return '', 200
    
    def __handle_start(self, chat_id):
    ...
    def __handle_close(self, chat_id):
        player = mm.get_player(chat_id)
        if player != None:
    ...
    def __handle_move(self, chat_id, text):
        player = mm.get_player(chat_id)
        if player.opponent != None:
